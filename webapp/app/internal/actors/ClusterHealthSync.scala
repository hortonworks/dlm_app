...
class ClusterHealthSync(val storage: ClusterDataStorage, ws: WSClient)
    extends Actor
    with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case GenRandom() =>
      storage
        .loadClusterInfos()
        .map { ambariClusters =>
          ambariClusters.foreach { ac =>
            // Generate Random load averages
            ac.clusters.foreach { cls =>
              cls.foreach { cl =>
                val avgLoad = r.nextDouble()
                val metric = ClusterMetric(cl.name,
                                           cl.ambariHost,
                                           cl.dataCenter,
                                           avgLoad,
                                           0,
      ...
      storage
        ...
        .map { ambariClusters =>
          ambariClusters.map { ac =>
            ...
            val prefix =
              s"${ac.ambari.protocol}://${ac.ambari.host}:${ac.ambari.port}/"
            ac.clusters.map { cls =>
              cls.map { cl =>
                ...
                for {
                  ...
                  val metrics = Try((response \ "metrics" \ "load" \ "1-min")
                      .as[List[List[Double]]]) getOrElse {
                    Logger.error(
                      s"Cannot parse metrics data with response ${response}")
                    List[List[Double]]()
                  }
                  val flatList = metrics.flatten
                  val loads = flatList.zipWithIndex.collect {
