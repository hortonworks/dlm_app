...
import com.hortonworks.dlm.beacon.domain.RequestEntities.S3CloudCredRequest
...
import models.GCSEntities.GCSAccountDetails
        ...
        Right(CloudAccountsBody(cloudAccounts.map(cloudAccount => {
          ...
          val accountDetails = CloudAccountProvider.withName(cloudAccount.accountDetails.provider) match {
            ...
            case CloudAccountProvider.GCS =>
              ...
              GCSAccountDetails(accountDetails.provider, Some(CloudCredentialType.withName(credentials.credentialType)),
