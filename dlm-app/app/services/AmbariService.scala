        ...
        val allClusterAmbariUserFuture: Future[Seq[AmbariUserReadPrivelege]] =
          Future.sequence(dataplaneClusters.map(cluster => {
            ...
            ambariService.requestAmbariApi(clusterId,
              ...
              case Left(error) => AmbariUserReadPrivelege(clusterId, false)
              case Right(res) =>
                res.validate[AmbariUserInfo] match {
                case JsSuccess(result, _) => AmbariUserReadPrivelege(result.id, true)
                case JsError(error) => AmbariUserReadPrivelege(clusterId, false)
              }
            }
          }))
