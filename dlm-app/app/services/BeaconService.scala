...
class BeaconService @Inject()(
  ...
  def validateAndSubmitCloudReplicationPolicy(clusterId: Long, beaconUrl: String, cloudCredName: String,
                                              ...
                                              policyRequest: AnyRef, p: Promise[Either[BeaconApiErrors, PostActionResponse]])
    ...
    getAllPolicies(queryString).map {
      case Right(policiesDetailsResponse) =>
        ...
        val targetDataSetList = targetDataSet.stripPrefix(s3DataSetPrefix).split("/")
        val sameTargetPathPolicyOption = allCloudFsPolicies.find(x => {
          val policyTargetDataSetList = x.targetDataset.stripPrefix(s3DataSetPrefix).split("/")
  ...
  def createReplicationPolicy(clusterId: Long, beaconUrl: String, cloudCredName: String, cloudCredResponse: Seq[CloudCredResponse],
                              policySubmitDefinition: PolicyDefinitionRequest, p: Promise[Either[BeaconApiErrors, PostActionResponse]])
                           (implicit token:Option[HJwtToken]) : Unit = {
  ...
  def testReplicationPolicy(clusterId: Long, beaconUrl: String, cloudCredName: String, cloudCredResponse: Seq[CloudCredResponse],
                                 policyTestRequest: PolicyTestRequest, p: Promise[Either[BeaconApiErrors, PostActionResponse]])
                (implicit token:Option[HJwtToken]) : Unit = {
  ...
  def addCloudAccount(cloudAccount: CloudAccountWithCredentials)
    ...
    getAllCloudCreds(queryString).map {
      ...
      case Right(cloudCredsDetailResponse) =>
        ...
        val filteredCloudCreds : Seq[CloudCredResponse] = cloudCredsDetailResponse.allCloudCreds.flatMap(x => x.cloudCreds.cloudCred).filter(item => item.name == cloudAccount.id)
...
object BeaconService {
  ...
  def sameTargetPathPolicyMsg (s3path: String, policyName: String) : String = "S3 path conflicts with the destination path " + s3path + " of existing policy " + policyName
