...
class Clusters @Inject()(
  ...
  def listBeaconStaleStatus() = AuthenticatedAction.async { request =>
    Logger.info("Received get cluster stale status request")
    implicit val token = request.token
    beaconService.getAllStaleClusters map {
      case Left(errors) => InternalServerError(JsonResponses.statusError(s"Failed with ${Json.toJson(errors)}"))
      case Right(staleClusters) => Ok(Json.toJson(staleClusters))
    }
  }
  ...
  def syncCluster(clusterId: Long) = AuthenticatedAction.async { request =>
    implicit val token = request.token
    beaconService.updateClusterDefinition(clusterId) map {
      case Left(errors) => InternalServerError(JsonResponses.statusError(s"Failed with ${Json.toJson(errors)}"))
      case Right(clusterUpdateResponse) => Ok(Json.toJson(clusterUpdateResponse))
    }
  }
}
