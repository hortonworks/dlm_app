...
object WASBEntities {
  ...
  case class WASBAccountDetails(provider: String, credentialType: Option[CloudCredentialType], accountName: Option[String])
    extends Serializable with CloudAccountDetails

  @SerialVersionUID(132)
  case class WASBAccountCredential(credentialType: String, accessKey: String)
    extends Serializable with CloudAccountCredentials
  @SerialVersionUID(133)
  case class WASBAccountCredentialSAS(credentialType: String, token: String)
    extends Serializable with CloudAccountCredentials
  ...
  case class BlobListResponse(fileList: Seq[BlobListItem], provider: CloudAccountProvider = CloudAccountProvider.WASB)
