...
object Entities {
  ...
  // DLM App meta properties
  case class DlmMetaProperties(version: String, policiesCount: Long, eventsCount: Long)
  ...
  case class BeaconCluster(id: Long, name: String, dataCenter: String, description: String, ambariWebUrl: String,
                          stats: Option[ClusterStats], totalHosts: Option[Long], location: Location, beaconUrl: String,
                          stackMajorVersion: String, stackVersion: Option[String])
  ...
  case class ClusterDefinitionDetails (cluster:Cluster, dpCluster: DataplaneCluster, nnClusterService : Map[String, Option[String]],
                                       hiveServerService : Map[String, Option[String]], sharedService: SharedServicesDetails,
                                       clusterDefinitions: Seq[PairedCluster], pairedClusterRequest:PairClusterRequest)

  case class BeaconAllClusters(unreachableBeacon: Seq[BeaconApiErrors] = Seq(), response: Seq[BeaconEntitiesWithClusterIdResponse])

  case class BeaconUpdateClusterResponse(unreachableBeacon: Seq[BeaconApiErrors] = Seq(), response: Seq[PostActionClusterResponse])
  ...
  case class PairedClusterDetails(cluster1: BeaconCluster, cluster2: BeaconCluster, status: String, message: Option[String])

  // Response schema for Pair cluster request
  case class PairedClustersResponse(unreachableBeacon: Seq[BeaconApiErrors] = Seq(), pairedClusters: Set[PairedClusterDetails] = Set())
  ...
  case class PoliciesDetails(policyId: String, name: String, description: Option[String], `type`: String,
                             ...
                             jobs: Seq[PolicyInstanceResponse], report: PolicyReport, plugins: Option[Seq[String]])
...
object JsonFormatters {
  ...
  implicit val pairedClusterDetailsWrites = Json.writes[PairedClusterDetails]
  ...
  implicit val dlmMetaProperties = Json.format[DlmMetaProperties]
  implicit val beaconAllClusters = Json.format[BeaconAllClusters]
  implicit val beaconUpdateClusterResponse = Json.format[BeaconUpdateClusterResponse]
