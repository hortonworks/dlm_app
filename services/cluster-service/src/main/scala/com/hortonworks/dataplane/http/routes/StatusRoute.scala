...
class StatusRoute @Inject()(val ws: WSClient,storageInterface: StorageInterface, val config: Config) extends BaseRoute{
  ...
  def checkAmbariAvailability(ep: AmbariEndpoint):Future[WSResponse] = {
    val endpoint: String = s"${ep.url}${Try(config.getString("dp.service.ambari.cluster.api.prefix")).getOrElse("/api/v1/clusters")}"
    val timeout = Try(config.getInt("dp.service.ambari.status.check.timeout.secs")).getOrElse(20)
    for {
      ...
      response <- ws.url(endpoint).withAuth(user.get,pass.get,WSAuthScheme.BASIC).withRequestTimeout(timeout seconds).get()
    } yield {
      response
    }
  }
