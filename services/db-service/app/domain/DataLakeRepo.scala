...
class DataLakeRepo @Inject()(private val userRepo: UserRepo, protected val dbConfigProvider: DatabaseConfigProvider) {
  ...
  def insert(datalake: Datalake): Future[Datalake] = db.run {
    val toPersist = Datalake(name = datalake.name, description = datalake.description, location = datalake.location, createdBy = datalake.createdBy, properties = datalake.properties)
  ...
  private class LocationsTable(tag: Tag) extends Table[Location](tag, Some("dataplane"), "dp_locations") {
    ...
    def * = (id, country, city) <> ((Location.apply _).tupled, Location.unapply)
  }


  private class DataLakesTable(tag: Tag) extends Table[Datalake](tag, Some("dataplane"), "dp_datalakes") {
    def id = column[Option[Long]]("id", O.PrimaryKey, O.AutoInc)
    ...
    def * = (id, name, description, locationId, userId, properties, created, updated) <> ((Datalake.apply _).tupled, Datalake.unapply)
  }
