...
class BeaconAdminServiceImpl()(implicit ws: KnoxProxyWsClient) extends BeaconAdminService {
  ...
  private def mapToUserDetailsResponse(res: WSResponse) = {
    res.status match {
      case 200 =>
        res.json.validate[UserDetailsResponse] match {
  ...
  override def getUserDetails(beaconEndpoint : String, clusterId: Long)(implicit token:Option[HJwtToken])
  : Future[Either[BeaconApiErrors, UserDetailsResponse]] = {
    ws.url(s"${urlPrefix(beaconEndpoint)}/user", clusterId, BEACON).withHeaders(token)
      .withAuth(user, password, WSAuthScheme.BASIC)
      .get.map(mapToUserDetailsResponse).recoverWith {
      case e: Exception => Future.successful(Left(BeaconApiErrors(SERVICE_UNAVAILABLE, Some(beaconEndpoint), Some(BeaconApiError(e.getMessage)))))
    }
  }
