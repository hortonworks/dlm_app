...
object WebService {
  ...
  trait ClientService {
    ...
    protected def mapToPostActionResponse(res: WSResponse, clusterId: Long) = {
      res.status match {
        case 200 =>
    ...
    protected def serializeObject[T <: AnyRef](data : T) = {
      data.getClass.getDeclaredFields.foldLeft("") { (acc, field) =>
        field.setAccessible(true)
        val fieldValue = field.get(data)
        fieldValue match {
          case Some(value) => acc + s"${field.getName.replaceAll("\\$u002E", ".")}=$value\n"
          case None => acc
          case _ => acc + s"${field.getName.replaceAll("\\$u002E", ".")}=$fieldValue\n"
        }
      }.stripLineEnd
    }
  ...
  trait BeaconClusterService extends ClientService {
    ...
    def listClusters(beaconEndpoint : String, clusterId: Long, dcClusterName: String)
    ...
    def deleteClusterDefinition(beaconEndpoint : String, clusterId: Long, dataCenterClusterName : String)
                               (implicit token:Option[HJwtToken]): Future[Either[BeaconApiErrors, PostActionClusterResponse]]
    def updateClusterDefinition(beaconEndpoint : String, clusterId: Long, dataCenterClusterName : String,  clusterDefinitionRequest: ClusterDefinitionRequest)
                               (implicit token:Option[HJwtToken]): Future[Either[BeaconApiErrors, PostActionClusterResponse]]
  }
  ...
  trait BeaconPolicyService extends ClientService {
    ...
    def submitAndSchedulePolicy(beaconEndpoint : String, clusterId: Long, policyName : String, policyDefinitionRequest : PolicyDefinitionRequest)
                               (implicit token:Option[HJwtToken]): Future[Either[BeaconApiErrors, PostActionClusterResponse]]
    def updatePolicy(beaconEndpoint : String, clusterId: Long, policyName : String, policyUpdateRequest : PolicyUpdateRequest)
                               (implicit token:Option[HJwtToken]): Future[Either[BeaconApiErrors, PostActionClusterResponse]]
