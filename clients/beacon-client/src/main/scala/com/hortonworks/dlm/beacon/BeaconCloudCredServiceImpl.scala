...
import com.hortonworks.dlm.beacon.domain.RequestEntities.{CloudCredRequest, GcsCloudCredRequest, S3CloudCredRequest, WASBCloudCredRequest}
...
class BeaconCloudCredServiceImpl()(implicit ws: KnoxProxyWsClient) extends BeaconCloudCredService {
  ...
  private def mapToCloudCredRequest(cloudCredRequest : CloudCredRequest) = {
    cloudCredRequest.authtype match {
      ...
      case "GCS_PRIVATEKEY" =>
        val gcsCloudCredRequest = cloudCredRequest.asInstanceOf[GcsCloudCredRequest]
        serializeObject(gcsCloudCredRequest)
    }
  }
