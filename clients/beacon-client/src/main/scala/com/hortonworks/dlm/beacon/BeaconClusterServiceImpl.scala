...
class BeaconClusterServiceImpl()(implicit ws: KnoxProxyWsClient) extends BeaconClusterService {
  ...
  private def mapToClusterDefinitionRequest(clusterDefinitionRequest:ClusterDefinitionRequest) : String = {
    ...
    val atlasConfigs  = clusterDefinitionRequest.sharedServiceConfigs.atlasServiceDetails match {
      case None => ""
      case Some(atlasServiceDetails) =>
        serializeObject(atlasServiceDetails)
    }

    val rangerConfigs =  clusterDefinitionRequest.sharedServiceConfigs.rangerServiceDetails match {
      case None => ""
      case Some(rangerServiceDetails) => {
        "rangerEndPoint = " + rangerServiceDetails.rangerEndPoint + "\n" +
        (rangerServiceDetails.rangerHDFSServiceName match {
          case Some(rangerHDFSServiceName) => "rangerHDFSServiceName = " + rangerHDFSServiceName + "\n"
          case None => ""
        }) +
  ...
  private def mapToUpdateClusterDefinitionRequest(clusterDefinition: ClusterDefinitionRequest): String = {
    val definition = mapToClusterDefinitionRequest(clusterDefinition)
    val omitProperties = Set("local")
    definition.split('\n').filterNot(x => omitProperties.contains(x.split('=')(0).trim)).mkString("\n")
  }
  ...
  override def listCluster(beaconEndpoint : String, clusterId: Long, clusterName: String)
    ...
    ws.url(s"${urlPrefix(beaconEndpoint)}/cluster/getEntity/$clusterName", clusterId, BEACON).withHeaders(token)
      ...
      case e: Exception => Future.successful(Left(BeaconApiErrors(SERVICE_UNAVAILABLE, Some(beaconEndpoint),
        Some(BeaconApiError(e.getMessage)))))
    }
  }

  override def listClusters(beaconEndpoint : String, clusterId: Long, dcClusterName: String)
                          (implicit token:Option[HJwtToken]): Future[Either[BeaconApiErrors, BeaconEntitiesWithClusterIdResponse]] = {
    val queryString = collection.immutable.HashMap("fields" -> "all")
    ws.url(s"${urlPrefix(beaconEndpoint)}/cluster/list", clusterId, BEACON).withHeaders(token)
      ...
      res.status match {
        case 200 =>
          res.json.validate[BeaconEntitiesResponse] match {
            case JsSuccess(result, _) =>
              Right(BeaconEntitiesWithClusterIdResponse(clusterId, beaconEndpoint, dcClusterName, result))
            case JsError(error) =>
              val url = Some(res.asInstanceOf[AhcWSResponse].ahcResponse.getUri.toUrl)
              Left(BeaconApiErrors(BAD_GATEWAY, url, Some(BeaconApiError(error.toString()))))
          }
  ...
  Future[Either[BeaconApiErrors, PostActionClusterResponse]] = {
      ...
      res.status match {
        case 200 =>
          res.json.validate[PostActionResponse] match {
            case JsSuccess(result, _) => Right(PostActionClusterResponse(clusterId, result))
            case JsError(error) => {
              Left(BeaconApiErrors(BAD_GATEWAY, url, Some(BeaconApiError(error.toString()))))
            }
          }
  ...
  Future[Either[BeaconApiErrors, PostActionClusterResponse]] = {
    ws.url(s"${urlPrefix(beaconEndpoint)}/cluster/delete/$dataCenterClusterName", clusterId, BEACON).withHeaders(token)
      .withAuth(user, password, WSAuthScheme.BASIC)
      .withHeaders(httpHeaders.toList: _*)
      .delete().map(x => mapToPostActionResponse(x, clusterId)).recoverWith {
      case e: Exception => Future.successful(Left(BeaconApiErrors(SERVICE_UNAVAILABLE, Some(beaconEndpoint), Some(BeaconApiError(e.getMessage)))))
    }
  }

  override def updateClusterDefinition(beaconEndpoint : String, clusterId: Long, dataCenterClusterName : String, clusterDefinitionRequest: ClusterDefinitionRequest)
                                      (implicit token:Option[HJwtToken]): Future[Either[BeaconApiErrors, PostActionClusterResponse]] = {
    val requestData:String =  mapToUpdateClusterDefinitionRequest(clusterDefinitionRequest)
    ws.url(s"${urlPrefix(beaconEndpoint)}/cluster/$dataCenterClusterName", clusterId, BEACON).withHeaders(token)
      .withAuth(user, password, WSAuthScheme.BASIC)
      .withHeaders(httpHeaders.toList: _*)
      .put(requestData)
      .map(x => mapToPostActionResponse(x, clusterId))
      .recoverWith {
        case e: Exception => Future.successful(Left(BeaconApiErrors(SERVICE_UNAVAILABLE, Some(beaconEndpoint), Some(BeaconApiError(e.getMessage)))))
      }
  }
