...
import {
  ...
  AddCloudStoreRequestBodyForWASB, AddCloudStoreRequestBodyForGCS
...
import { jsonValidator, uniqValidator } from 'utils/form-util';
...
import { REPLICATION_ADLS_SUPPORT, REPLICATION_GCS_SUPPORT, REPLICATION_WASB_SUPPORT } from 'models/features.model';
...
export class AddCloudFormComponent implements OnInit, OnChanges, OnDestroy {
  ...
  gcsCredFileIsRead = false;
  requiredFieldsForGcsCredsFile: string[] = ['client_email', 'private_key_id', 'private_key'];

  get validationMsgForGcsCredsFile() {
    return this.t.instant('forms.validation_errors.jsonValidator', {fields: this.requiredFieldsForGcsCredsFile.join(', ')});
  }
  ...
  get cloudProviderOptions() {
    return this.CLOUD_PROVIDER_VALUES.reduce((acc, cloudProvider) => {
      ...
      if (cloudProvider === GCS && this.features.isDisabled(REPLICATION_GCS_SUPPORT)) {
        return acc;
      }
      return acc.concat({
  ...
  private serializeValueForGCS(formValue: AbstractControl): AddCloudStoreRequestBodyForGCS {
    const credentialType = CREDENTIAL_TYPES.GCS.GCS_PRIVATEKEY;
    const provider = this.cloudProviderType;
    return {
      id: formValue.get('credentialName').value.trim(),
      accountDetails: {
        provider,
        accountEmail: formValue.get('gcs.accountEmail').value.trim()
      },
      accountCredentials: {
        credentialType,
        accessKeyId: formValue.get('gcs.accessKeyId').value.trim(),
        secretAccessKey: formValue.get('gcs.secretAccessKey').value.trim()
      }
    };
  }
  ...
  public initForm() {
    ...
    this.addCloudForm = this.formBuilder.group({
      ...
      gcs: this.formBuilder.group({
        ...
        credsFile: [''],
        credsFileContent: ['', jsonValidator(this.requiredFieldsForGcsCredsFile)]
      })
    });
    this.patchFormState();
  ...
  patchFormState() {
    if (this.isEditMode) {
      ...
      if (this.isS3Selected) {
        ...
        this.disableFieldsFor(['adls', 'wasb', 'gcs']);
      }
      if (this.isAdlsSelected) {
        this.enableFieldsFor('adls');
        this.disableFieldsFor(['s3', 'wasb', 'gcs']);
      }
      if (this.isWasbSelected) {
        this.enableFieldsFor('wasb');
        this.disableFieldsFor(['s3', 'adls', 'gcs']);
      }
      if (this.isGcsSelected) {
        this.enableFieldsFor('gcs');
        this.disableFieldsFor(['s3', 'adls', 'wasb']);
      }
    } else {
  ...
  get isGcsSelected(): boolean {
    return this.addCloudForm.get('cloudProviderType').value === CLOUD_PROVIDERS.GCS;
  }
  ...
  saveButtonHandler() {
    ...
    if (this.cloudProviderType === CLOUD_PROVIDERS.S3) {
      ...
      this.saveAccount(requestPayload);
    } else if (this.cloudProviderType === CLOUD_PROVIDERS.GCS) {
      this.isSaveInProgress = true;
      const requestPayload = this.serializeValueForGCS(this.addCloudForm);
      this.saveAccount(requestPayload);
    }
  }
  ...
  readGcsCredsFile(e) {
    ...
    const gcs = this.addCloudForm.get('gcs');
    this.gcsCredFileIsRead = false;
    const emptyFields = {
      credsFileContent: '',
      accountEmail: '',
      accessKeyId: '',
    ...
    if (file) {
      readFile(file).subscribe(text => {
        this.gcsCredFileIsRead = true;
        try {
          // escape new line characters that are present in private_key
          const escapedFileContent = text.replace(/\\n/g, '\\\\n');
          const credsFileContent = JSON.parse(escapedFileContent);
          gcs.patchValue({
            credsFileContent: text,
            accountEmail: credsFileContent.client_email,
            accessKeyId: credsFileContent.private_key_id,
            secretAccessKey: credsFileContent.private_key
          });
        } catch (e) {
          gcs.patchValue(emptyFields);
        }
        this.cdRef.detectChanges();
      });
    } else {
  ...
  private subscribeToProviderType() {
    const subscription = this.addCloudForm.get('cloudProviderType').valueChanges.subscribe(type => {
      ...
      if (this.isS3Selected) {
        toEnable = 's3';
        toDisable = ['adls', 'wasb', 'gcs'];
      } else if (this.isAdlsSelected) {
        toEnable = 'adls';
        toDisable = ['s3', 'wasb', 'gcs'];
      } else if (this.isWasbSelected) {
        toEnable = 'wasb';
        toDisable = ['s3', 'adls', 'gcs'];
      } else if (this.isGcsSelected) {
        toEnable = 'gcs';
        toDisable = ['s3', 'adls', 'wasb'];
      }
