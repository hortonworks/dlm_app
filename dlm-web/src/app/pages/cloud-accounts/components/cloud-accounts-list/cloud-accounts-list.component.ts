...
export class CloudAccountsListComponent implements OnInit {
  ...
  cloudAccountActions = [
    ...
    },
    {
      ...
      disabled: function (account) {
        return hasUnexpiredPolicies(account) || this.userService.isUserReadOnly;
      }.bind(this),
      disabledMessage: this.userService.isUserReadOnly ?
        this.userService.userIsReadOnlyMessage :
        'page.cloud_stores.content.accounts.delete.expired_account_hint'
    }
  ];
  ...
  getTooltipForDelete(account) {
    if (this.userService.isUserReadOnly) {
      return this.userService.userIsReadOnlyMessage;
    }
    return hasUnexpiredPolicies(account) ?
      this.t.instant('page.cloud_stores.content.accounts.delete.expired_account_hint') :
      this.t.instant('common.delete');
  }
