...
import { PAIRING_STATUS } from 'constants/pairing.constant';
...
export class PairingCardListComponent implements OnInit {
  ...
  get activePairings(): Pairing[] {
    return this.pairings.filter(pair => pair.status === PAIRING_STATUS.PAIRED);
  }

  get suspendedPairings(): Pairing[] {
    return this.pairings.filter(pair => pair.status === PAIRING_STATUS.SUSPENDED);
  }
