...
export class JobsTableComponent implements OnInit {
  ...
  rowActions = <ActionItemType[]>[
    {
      label: 'Abort',
      name: 'ABORT',
      disableFn: this.isAbortDisabled.bind(this),
      qeAttr: 'abort-job'
    },
  ...
  isRerunDisabled(job, _): boolean {
    ...
    return !lastJob || lastJob.id !== job.id || this.cannotRerun(this.policy, lastJob) || this.userService.isUserReadOnly;
  }

  isAbortDisabled(job, _): boolean {
    return job.status !== JOB_STATUS.RUNNING || this.userService.isUserReadOnly;
  }
