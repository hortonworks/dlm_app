...
import { filterClustersByTDE, filterClustersByHdfsCloud, filterClustersByHdfsWasbCloud } from 'utils/cluster-util';
...
export class StepDestinationComponent implements OnInit, OnDestroy, StepComponent {
  ...
  get isCloudReplication(): boolean {
    return contains([SOURCE_TYPES.S3, SOURCE_TYPES.WASB], this.form.get('destination.type').value) ||
  ...
  get isCloudType(): boolean {
    return contains([SOURCE_TYPES.S3, SOURCE_TYPES.WASB], this.destinationType);
  }
  ...
  get destinationTypes() {
    ...
    if (this.general.type === POLICY_TYPES.HIVE || this.source.type === SOURCE_TYPES.S3 || this.source.type === SOURCE_TYPES.WASB) {
      return onlyCluster;
    }
    const replicationWasbSupported = filterClustersByHdfsWasbCloud(this.clusters).some(c => c.id === sourceClusterId);
    const replicationCloudFS = filterClustersByHdfsCloud(this.clusters).some(c => c.id === sourceClusterId);
    return replicationWasbSupported ? [cluster, s3, wasb] : replicationCloudFS ? [cluster, s3] : onlyCluster;
  }
  ...
  private subscribeToDestinationType() {
    ...
    const subscription = destinationControls.type.valueChanges.subscribe(type => {
      ...
      } else if (type === SOURCE_TYPES.WASB) {
