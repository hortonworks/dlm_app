...
export class StepSourceComponent implements OnInit, AfterViewInit, OnDestroy, StepComponent {
  ...
  get isSourceCloudType() {
    return contains([SOURCE_TYPES.S3, SOURCE_TYPES.WASB], this.sourceType);
  }
  ...
  get sourceTypes() {
    ...
    return this.isHivePolicy() || !clustersWithCloudHdfs.length ? [cluster] : [cluster, s3, wasb];
  }
  ...
  private setupDatabaseChanges(form: FormGroup): void {
    ...
    const databases$ = this.selectedSource$.pipe(switchMap(sourceCluster => {
    ...
    this.sourceDatabases$ = observableCombineLatest(this.databaseSearch$, databases$).pipe(
