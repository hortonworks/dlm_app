...
import {
  POLICY_REPEAT_MODES, POLICY_TIME_UNITS, POLICY_START,
  POLICY_DAYS,
  WIZARD_STEP_ID
...
export class StepScheduleComponent implements OnInit, OnDestroy, StepComponent {
  ...
  readonly stepId = WIZARD_STEP_ID.SCHEDULE;
  ...
  get defaultTime(): Date {
    return this.policyWizardService.defaultStartTime();
  }

  get defaultEndTime(): Date {
    return this.policyWizardService.defaultEndTime();
  }
  ...
  get shouldShowStartField() {
    return !this.isEditMode || this.isEditMode && !this.policyAlreadyRun;
  }
  ...
  private setupEditMode(): void {
    if (this.policy) {
      ...
      this.policyWizardService.patchFormByStep(this.form, WIZARD_STEP_ID.SCHEDULE, this.policy);
    }
  }
  ...
  private setupUnitChanges(form: FormGroup) {
    const unitChanges$: Observable<string> = form.get('job.unit').valueChanges.pipe(distinctUntilChanged());
  ...
  private setupStartModeChanges(form: FormGroup) {
    const startModeChanges$: Observable<string> = form.get('job.start').valueChanges.pipe(distinctUntilChanged());
  ...
  validateTime = (formGroup: FormGroup) => {
    ...
    if (dateFieldValue && this.startOption === this.policyStart.ON_SCHEDULE) {
      ...
      if (!this.policyAlreadyRun && dateWithTime.isBefore(moment())) {
