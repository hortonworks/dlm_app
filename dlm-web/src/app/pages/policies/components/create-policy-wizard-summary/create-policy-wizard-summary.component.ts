...
export class WizardSummaryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit () {
    ...
    const dataChanges$ = observableCombineLatest(stepValues$, wizardSteps$);
    const loadSummary$ = dataChanges$.pipe(
        ...
        if (this.showSummaryTree) {
          ...
          if (this.shouldShowStepSummaryFor(this.WIZARD_STEP_ID.SOURCE, stepValues, steps)) {
            ...
            if (source) {
              let name = '';
              let path = '';
              const iconClass = contains([SOURCE_TYPES.S3, SOURCE_TYPES.WASB], source['type']) ? 'fa-cloud' : 'fa-cube';
              ...
              } else if (contains([SOURCE_TYPES.S3, SOURCE_TYPES.WASB], source['type'])) {
                name = source['cloudAccount'] || '';
                const prefix = source['type'] === SOURCE_TYPES.WASB ? 'wasb://' : 's3://';
                path =  prefix + source['s3endpoint'];
              }
              this.summaryItems.push({
          ...
          if (this.shouldShowStepSummaryFor(this.WIZARD_STEP_ID.DESTINATION, stepValues, steps)) {
            ...
            if (destination) {
              let name = '';
              let path = '';
              const iconClass = contains([SOURCE_TYPES.S3, SOURCE_TYPES.WASB], destination['type']) ? 'fa-cloud' : 'fa-cube';
              if (destination['type'] === SOURCE_TYPES.CLUSTER && 'cluster' in destination) {
                ...
                const prefix = destination['type'] === SOURCE_TYPES.WASB ? 'wasb://' : 's3://';
                path = prefix + destination['s3endpoint'];
              }
              this.summaryItems.push({
