...
import { interval as observableInterval, combineLatest as observableCombineLatest, Observable, Subscription,
  BehaviorSubject } from 'rxjs';
import {map, delay, skip, tap, take, withLatestFrom, startWith,  distinctUntilChanged, filter } from 'rxjs/operators';
...
import { getEventsWithIssues } from 'selectors/event.selector';
...
export class OverviewComponent extends PageComponent implements OnInit, OnDestroy {
  ...
  constructor(private store: Store<fromRoot.State>,
    ...
    super();
    this.events$ = store.select(getEventsWithIssues);
    ...
    this.fullfilledClusters$ = this.clusters$.pipe(
    ...
    this.unhealthyClusters$ = this.store.select(getUnhealthyClusters).pipe(
    ...
    this.unhealthyPolicies$ = this.store.select(getUnhealthyPolicies).pipe(
    ...
    this.clustersMapData$ = observableCombineLatest(this.clusters$,
      store.select(getCountPoliciesForSourceClusters), this.lowCapacityClusters$).pipe(
      startWith([[], [], []]),
      map(([clusters, policiesCount, lowCapacityClusters]) => this.makeClustersMapData(clusters, policiesCount, lowCapacityClusters)));
    this.clusterLegend$ = observableCombineLatest(this.clustersMapData$, this.selectedCluster$, this.policies$).pipe(
    ...
    this.shouldShowTable$ = this.overallProgress$.pipe(
  ...
  private initPolling() {
    const polling$ = observableInterval(POLL_INTERVAL).pipe(
      ...
      tap(([_, clusters]) => {
        [
          loadPolicies({numResults: this.policiesQueryCount, instanceCount: 10}),
          loadClusters(),
          loadEvents()
  ...
  ngOnInit() {
    const policiesCountSubscription = this.store.select(getDlmAppProperties).pipe(
      ...
      distinctUntilChanged()
    ).subscribe(dlmProperties => {
      [
        loadPolicies({numResults: dlmProperties.policiesCount, instanceCount: 10}, {requestId: POLICIES_REQUEST}),
        ...
        loadEvents({numResults: dlmProperties.eventsCount}),
        loadBeaconCloudCreds({requestId: CLOUD_CREDENTIALS_REQUEST}),
        loadAccounts()
      ].map(action => this.store.dispatch(action));
      this.policiesQueryCount = dlmProperties.policiesCount;
    });
    const overallProgressSubscription = this.completedRequest$(this.overallProgress$).pipe(
      take(1),
      tap(_ => this.initPolling()), )
      .subscribe();
    this.tableData$ = observableCombineLatest(this.tableResources$, this.jobStatusFilter$).pipe(
