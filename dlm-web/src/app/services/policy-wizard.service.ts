...
import { Policy, PolicyDefinition, PolicyUpdatePayload, PolicyPlugin } from 'models/policy.model';
...
import { omitEmpty, isEmpty, multiLevelResolve } from 'utils/object-utils';
...
import { gte } from '../utils/version-util';
...
export class PolicyWizardService {
  ...
  private mapAdvancedValues(policy: Policy): StepAdvancedValue {
    return {
      advanced: {
        ...
        disable_atlas: !contains(policy.activePlugins, PolicyPlugin.Atlas)
      }
    };
  }
  ...
  serializePolicy(formsData: CreatePolicyFormState, dependencies: PolicySerializeDependencies): SerializedPolicy {
    ...
    const isHiveCloud = general.type === POLICY_TYPES.HIVE && contains([UnderlyingFsForHive.S3,
      UnderlyingFsForHive.WASB, UnderlyingFsForHive.GCS], getUnderlyingHiveFS(dc));
    if (destination.type === SOURCE_TYPES.CLUSTER) {
      ...
      clusterId = sc.id;
      if (contains([SOURCE_TYPES.S3, SOURCE_TYPES.WASB, SOURCE_TYPES.GCS], destination.type)) {
        // destination s3
    ...
    if (source.type === SOURCE_TYPES.CLUSTER) {
      ...
      if (contains([SOURCE_TYPES.S3, SOURCE_TYPES.WASB, SOURCE_TYPES.GCS], source.type)) {
        // source s3
    ...
    if (advanced.disable_atlas && isClusterWithAtlas(sc) && isClusterWithAtlas(dc)) {
  ...
  supportsDifferentDestinationPath(cluster: Cluster): boolean {
    return gte(multiLevelResolve(cluster, 'beaconAdminStatus.beaconAdminStatus.dlmEngineVersion'), '1.1.0');
  }
}
