...
export class JobService {
  ...
  private doJobsRequest(url) {
    return this.httpClient.get<any>(url).pipe(map(response => {
      response.jobs = response.jobs.map(this.normalizeJob.bind(this));
      return response;
    }));
  }

  private getJobDuration(startTime, endTime) {
    return new Date(endTime).getTime() - new Date(startTime).getTime();
  }

  normalizeJob(job): Job {
    const duration = job.endTime && job.startTime ?
      this.getJobDuration(job.startTime, job.endTime) : -1;
  ...
  getJobsForPolicies(policies: Policy[], numResults = 1000): Observable<any> {
    const requests = policies.map(policy => this.getJobsForPolicy(policy, numResults).pipe(catchError(err => observableOf({jobs: []}))));
    return observableForkJoin(requests).pipe(map(responses => {
