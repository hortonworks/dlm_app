...
import { filter, distinctUntilKeyChanged, take, tap } from 'rxjs/operators';
...
export class AsyncActionsService {
  ...
  dispatch(action: ActionWithPayload<any>): Observable<ProgressState> {
    ...
    return this.onComplete(requestId).pipe(
      take(1),
      tap(_ => this.store.dispatch(removeProgressState(requestId)))
    );
  }
}
