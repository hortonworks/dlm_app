...
export class UserService {
  ...
  get userIsReadOnlyMessage(): string {
    const userName = this.user ? this.user.username : '';
    return this.t.instant('user.isReadOnly', {userName});
  }

  get userIsNotInfraAdminMessage(): string {
    const userName = this.user ? this.user.username : '';
    return this.t.instant('user.isNotInfraAdmin', {userName});
  }

  get isUserReadOnly(): boolean {
    const roles = this.user ? this.user.roles : [];
    return !contains(roles, this.ROLES.INFRAADMIN) &&
      !contains(roles, this.ROLES.DLM_ADMIN) &&
      contains(roles, this.ROLES.DLM_USER);
  }

  get isUserInfraAdmin(): boolean {
    return this.hasRole(this.ROLES.INFRAADMIN);
  }
  ...
  private hasRole(userRole): boolean {
