...
import {concatMap, take, switchMap, map, catchError} from 'rxjs/operators';
...
export class HdfsService {
  ...
  getFilesList(clusterId, filePath, filePathWithFilter = false): Observable<any> {
    filePath = filePath || '/';
    return this.store.select(getClustersWithBeacon).pipe(take(1), switchMap(clusters => {
      const cluster = clusters.find(c => c.id === clusterId);
      let query: any = {};
      if (filePathWithFilter) {
        const chunks = filePath.split('/');
        const filter = chunks.pop();
        query.filePath = chunks.join('/') || '/';
        if (filter) {
          query.filter = filter;
        }
      } else {
  ...
  checkFileEncryption(clusterId: number, filePathOrDirectory: string): Observable<ListStatus> {
    ...
    if (!contains(['', '/'], filePathOrDirectory)) {
      ...
      const selectDirectory = () => this.store.select(getAllFilesForClusterPath(clusterId, path)).pipe(
        map(dirs => findDir(dirs, fileName)),
        catchError(_ => observableOf({} as ListStatus)), );
      // performance optimization. Don't load directory list when path is '/'
      // because we might have it loaded
      if (path === '/') {
        return this.store.select(getAllFilesForClusterPath(clusterId, path)).pipe(
          switchMap(dirs => {
            if (dirs !== undefined) {
              return selectDirectory();
            } else {
              return this.asyncActions.dispatch(listFiles(clusterId, path)).pipe(
  ...
  checkSnapshottableAncestor(clusterId, filePathOrDirectory: string): Observable<ListStatus> {
    ...
    if (!contains(['', '/'], filePathOrDirectory)) {
      ...
      const selectDirectory = () => this.store.select(getAllFilesForClusterPath(clusterId, path)).pipe(
        ...
        take(1),
        catchError(_ => observableOf({} as ListStatus)), );
      const selectAndRecheck = () => selectDirectory().pipe(
        switchMap(dir => {
          if (!dir) {
            return observableOf(null);
          }
          if (dir.snapshottable) {
            return observableOf(dir);
          }
