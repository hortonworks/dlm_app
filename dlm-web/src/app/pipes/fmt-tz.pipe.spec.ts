...
describe('FmtTzPipe', () => {
  let injector, changeDetectorRef, timeZoneService, pipe, timestamp, format;
  ...
  beforeEach(() => {
    ...
    pipe = new FmtTzPipe(changeDetectorRef, timeZoneService);
  });
  ...
  describe('#Subscription', () => {
    ...
    it('should call `markForCheck`', () => {
      timeZoneService.userTimezoneIndex$.next('another value');
      expect(changeDetectorRef.markForCheck).toHaveBeenCalled();
    });

    it('should not call `markForCheck`', () => {
      timeZoneService.userTimezoneIndex$.next('');
  ...
  describe('#transform', () => {
    ...
    beforeEach(() => {
      timestamp = 100500;
      format = '';
      pipe.transform(timestamp, format);
      spyOn(timeZoneService, 'formatDateTimeWithTimeZone');
    });

    it('should recalculate', () => {
      pipe.transform(1, '');
      expect(timeZoneService.formatDateTimeWithTimeZone).toHaveBeenCalledWith(1, '');
    });

    it('should not recalculate', () => {
      pipe.transform(timestamp, format);
      expect(timeZoneService.formatDateTimeWithTimeZone).not.toHaveBeenCalled();
    });
