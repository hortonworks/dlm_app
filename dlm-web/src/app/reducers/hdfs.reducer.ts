...
export function reducer(state = initialState, action): State {
  switch (action.type) {
    ...
    case fromHdfs.ActionTypes.FILTER_FILES.SUCCESS:
      return filterFilesSuccess(state, action);
...
function listFilesSuccess(state = initialState, action): State {
  const parentFullPath = action.payload.meta.path;
  return storeFiles(state, action, parentFullPath);
}

function filterFilesSuccess(state = initialState, action): State {
  const parentFullPath = action.payload.meta.path.split('/').slice(0, -1).join('/') || '/';
  return storeFiles(state, action, parentFullPath);
}

function storeFiles(state = initialState, action, parentFullPath: string): State {
  ...
  const fileStatusEntities = fileStatus.reduce((entities: { [path: string]: ListStatus }, entity: ListStatus) => {
    return Object.assign({}, entities, {
      [entity.pathSuffix]: {...entity, parentFullPath}
    });
  }, {});
  ...
  return {
    lastRequestMeta: action.payload.response,
