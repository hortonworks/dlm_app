...
import { Step, StepState } from 'models/wizard.model';
...
export function reducer(state = initialState, action): State {
  switch (action.type) {
    ...
    case fromPolicy.ActionTypes.WIZARD_SAVE_STEP_VALUE:
      return wizardSaveStepValue(state, action);
    ...
    case fromPolicy.ActionTypes.WIZARD_UPDATE_STEP_STATE:
      return updateStepsState(state, action.payload.stepIds, action.payload.state);
    default:
      return state;
  }
}
...
function wizardSaveStepValue(state: State, action): State {
  const {stepId, value} = action.payload;
  const currentStepState = state.entities[stepId];
...
function updateStepsState(state, stepIds, status: StepState): State {
  return {
    ...state,
    entities: {
      ...state.entities,
