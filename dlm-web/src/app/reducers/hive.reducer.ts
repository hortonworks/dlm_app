...
export function reducer(state = initialState, action): State {
  switch (action.type) {
    ...
    case ActionTypes.LOAD_TABLES.SUCCESS:
      const tables = action.payload.response.tables as HiveTable[];
      const databaseId = tables.length ? tables[0].databaseEntityId : action.payload.meta.database;
      return {
        entities: {
          ...state.entities,
          [databaseId]: {
            ...state.entities[databaseId],
            isEncrypted: action.payload.response.isEncrypted,
            tables
          }
        }
      };
