...
import {delay, take, filter, distinctUntilKeyChanged, switchMap, repeat} from 'rxjs/operators';
...
export class LogModalDialogComponent implements OnInit {
  ...
  private startPolling(): void {
    ...
    this.polling = observableTimer(POLL_INTERVAL).pipe(
      switchMap(() => {
        ...
        return this.store.select(getProgressState(INTERNAL_LOG_REQUEST)).pipe(
  ...
  ngOnInit(): void {
    ...
    this.logService.getChangeEmitter()
      .subscribe(item => {
