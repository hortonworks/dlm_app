...
import {pluck, tap, distinctUntilChanged, switchMap, map} from 'rxjs/operators';
import {
  ChangeDetectorRef,
  Component,
  EventEmitter,
...
export class CloudContainerBrowserComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const dataChanges$ = observableCombineLatest(this.currentDirectory$, this.container$);
    ...
    this.rows$ = dataChanges$.pipe(
      switchMap(([path, container]) => {
        return container ? this.store.select(getAllFilesForCloudPath(container.id, path)).pipe(map(files => {
    ...
    this.spinner$ = observableCombineLatest(this.rows$, requestProgress$.pipe(pluck('isInProgress'))).pipe(
