...
export class TransferredObjectsComponent {
  ...
  get amount(): number {
    if (this.executionType === POLICY_EXECUTION_TYPES.HIVE) {
      return this.status === JOB_STATUS.SUCCESS ? Math.max(makeNumber(this.completed), makeNumber(this.total)) : makeNumber(this.completed);
    }
    return makeNumber(this.filesCopied);
  }
