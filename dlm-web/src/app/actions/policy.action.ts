...
import { type, requestType, createRequestAction } from 'utils/type-action';
import { ActionWithPayload } from 'actions/actions.type';
import { Policy, PolicyPayload, PolicyUpdatePayload } from 'models/policy.model';
import { ActionSuccess, ActionFailure } from 'utils/extended-actions.type';
import { StepName, StepState } from 'models/wizard.model';
...
export const ActionTypes = {
  ...
  UPDATE_POLICY: requestType('UPDATE_POLICY'),
  ...
  WIZARD_SAVE_STEP_VALUE: type('WIZARD_SAVE_STEP_VALUE'),
  ...
  WIZARD_UPDATE_STEP_STATE: type('WIZARD_UPDATE_STEP_STATE'),
...
export const wizardSaveStep = (stepId: StepName, value): ActionWithPayload<any> => ({
  type: ActionTypes.WIZARD_SAVE_STEP,
  payload: {stepId, value}
});

export const wizardSaveStepValue = (stepId: StepName, value): ActionWithPayload<any> => ({
  type: ActionTypes.WIZARD_SAVE_STEP_VALUE,
  payload: {stepId, value}
});
...
export const wizardResetStep = (stepId: StepName): ActionWithPayload<any> => ({
  type: ActionTypes.WIZARD_RESET_STEP,
  payload: {stepId}
});

export const wizardMoveToStep = (stepId: StepName): ActionWithPayload<any> => ({
  type: ActionTypes.WIZARD_MOVE_TO_STEP,
  payload: {stepId}
});

export const wizardUpdateStepState = (state: StepState, ...stepIds: StepName[]): ActionWithPayload => ({
  type: ActionTypes.WIZARD_UPDATE_STEP_STATE,
  payload: {stepIds, state}
});
...
export const {
  ...
  updatePolicySuccess,
  updatePolicyFailure
} = createRequestAction(ActionTypes.UPDATE_POLICY, {
  start: (policy: Policy, updatePayload: PolicyUpdatePayload, meta = {}) => ({
    meta,
    policy,
    updatePayload
  })
