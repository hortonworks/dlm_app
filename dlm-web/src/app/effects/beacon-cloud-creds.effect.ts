...
export class BeaconCloudCredEffects {
  ...
  loadBeaconCloudCreds$: Observable<any> = this.actions$
    .ofType(beaconActions.LOAD_BEACON_CLOUD_CREDS.START).pipe(
    map<Action, any>(toPayload),
    switchMap(payload => this.beaconService
      .fetchBeaconCloudCreds().pipe(
      map(result => loadBeaconCloudCredsSuccess(result, payload.meta)),
      catchError(err => observableOf(loadBeaconCloudCredsFailure(err, payload.meta))), )), );

  @Effect()
  loadBeaconCloudCredsWithPolicies$: Observable<any> = this.actions$
    .ofType(beaconActions.LOAD_BEACON_CLOUD_CREDS_WITH_POLICIES.START).pipe(
    map<Action, any>(toPayload),
    switchMap(payload => this.beaconService
      .fetchBeaconCloudCredsWithPolicies().pipe(
      map(result => loadBeaconCloudCredsWithPoliciesSuccess(result, payload.meta)),
      catchError(err => observableOf(loadBeaconCloudCredsWithPoliciesFailure(err, payload.meta))), )), );
