...
export class HiveListEffects {
  ...
  loadDatabases$: Observable<any> = this.actions$
    .ofType(hiveListActions.LOAD_DATABASES.START).pipe(
    map<Action, any>(toPayload),
    switchMap(payload => {
      const { clusterId, meta } = payload;
      return this.hiveService.fetchDatabases(clusterId).pipe(
        map(databases => loadDatabasesSuccess(databases, meta)),
        catchError(err => observableOf(loadDatabasesFail(err, meta))), );
    }), );

  @Effect()
  loadTables$: Observable<any> = this.actions$
    .ofType(hiveListActions.LOAD_TABLES.START).pipe(
    map<Action, any>(toPayload),
    switchMap(payload => {
      const { clusterId, databaseId, meta } = payload;
      return this.hiveService.fetchTables(clusterId, databaseId).pipe(
        map(tables => loadTablesSuccess(tables, meta)),
        catchError(err => observableOf(loadTablesFail(err, payload.meta))), );
    }), );

  @Effect()
  loadFullDatabases$: Observable<any> = this.actions$
    .ofType(hiveListActions.LOAD_FULL_DATABASES.START).pipe(
    map<Action, any>(toPayload),
    switchMap(payload => {
      const { clusterId, meta } = payload;
      return this.hiveService.fetchFullDatabases(clusterId).pipe(
        map(databases => loadDatabasesSuccess(databases, meta)),
        catchError(err => observableOf(loadDatabasesFail(err, meta))), );
    }), );
