...
export class PairingEffects {
  ...
  loadPairings$: Observable<any> = this.actions$
    .ofType(pairingActions.LOAD_PAIRINGS.START).pipe(
    map<Action, any>(toPayload),
    switchMap(payload => {
      return this.pairingService.fetchPairings().pipe(
        map(pairings => loadPairingsSuccess(pairings, payload.meta)),
        catchError(err => observableOf(loadPairingsFail(err, payload.meta))), );
    }), );

  @Effect()
  createPairing$: Observable<any> = this.actions$
    .ofType(pairingActions.CREATE_PAIRING.START).pipe(
    map<Action, any>(toPayload),
    switchMap(payload => {
      return this.pairingService.createPairing(payload.pairing).pipe(
        map(response => {
          return createPairingSuccess({...response, payload: payload.pairing}, payload.meta);
        }),
        catchError(err => observableOf(createPairingFail(err, payload.meta))), );
    }), );

  @Effect()
  deletePairing$: Observable<any> = this.actions$
    .ofType(pairingActions.DELETE_PAIRING.START).pipe(
    map<Action, any>(toPayload),
    switchMap(payload => {
      return this.pairingService.deletePairing(payload.pairing).pipe(
        map(response => {
          return deletePairingSuccess({...response, payload: payload.pairing}, payload.meta);
        }),
        catchError(err => observableOf(deletePairingFail(err, payload.meta))), );
    }), );
