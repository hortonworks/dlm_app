...
export type StepName = 'general' | 'source' | 'destination' | 'schedule' | 'advanced';
...
export interface Step<T = any> {
  ...
  value: T;
  nextStepId: string;
  previousStepId: StepName;
}

export interface CreatePolicyForm {
  general: StepGeneralValue;
  source: StepSourceValue;
  destination: StepDestinationValue;
  schedule: StepScheduleValue;
  advanced: StepAdvancedValue;
}
...
export interface SerializedPolicy {
  policyData: PolicyPayload;
  clusterId: number;
}

export interface PolicySerializeDependencies {
  clusters: Cluster[];
}
