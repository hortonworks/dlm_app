...
export const getAllPoliciesWithCloud = createSelector(getAllPoliciesWithClusters, getAllBeaconCloudCreds, (policies, credentials) => {
  ...
  return policies.map(policy => {
    ...
    const p = {
      ...policy,
      cloudCredentialResource: 'sourceCluster' in policy ? cloudCredential(policy.sourceClusterResource.id, cloudCred) || {} :
        cloudCredential(policy.targetClusterResource.id, cloudCred) || {},
