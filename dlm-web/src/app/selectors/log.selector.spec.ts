...
describe('Logs Selectors', () => {
  ...
  let state: State;
  beforeEach(() => {
    log01 = <Log>{requestId: '1', status: 'SUCCEEDED', message: 'event 1'};
    log02 = <Log>{requestId: '2', status: 'SUCCEEDED', message: 'event 2'};
    const logsState = {
      logs: {
        entities: {
          '/d1/c1/p1/000': log01,
  ...
  describe('#getAllLogs', () => {
    it('should map entities to array sorted by startTime', () => {
      ...
      expect(result[0]).toBe(log01);
      expect(result[1]).toBe(log02);
    });
  });
