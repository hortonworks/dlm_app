...
export const clusterSelector = createEntitySelector<Cluster>('Cluster', getClusters, {
  beaconConfigStatus: belongsTo('BeaconConfigStatus', {
    id: 'clusterId',
...
export const getClustersWithLowCapacity = createSelector(
  getAllClusters,
  (clusters: Cluster[]) => clusters.filter(cluster => cluster.stats &&
    (cluster.stats.CapacityRemaining / cluster.stats.CapacityTotal < 0.1)));
